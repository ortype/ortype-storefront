...
export default class OrtypeCatalogFont extends OrType {
  async init() {
    await super.init()
  ...
  async getFontDue() {
    return util.inspect(this.fontDue, {
      depth: 3,
      showHidden: true,
      getters: 'get',
    })
    ...
    return this.getFeatures().map(({ tag, feature }) => {
      return {
        ...
        lookups: feature.lookupListIndexes.map((lookupIndex) => {
          ...
          return this.instance.tables.gsub.lookups[lookupIndex].subtables.map(
            (subtable, i) => {
              ...
              switch (substFormat) {
                 ...
                 */
                  return subtable.alternateSets
                    ? subtable.alternateSets[0]
              ...
              return subtable.coverage
                ? subtable.coverage.glyphs
                : subtable.inputCoverage.glyphs
            }
          )
        }),
      }
