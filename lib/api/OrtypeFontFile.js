...
export default class OrtypeFontFile extends OrtypeAbstractFont {
  ...
  async shouldUpdate() {
    ...
    //   }]
    // })
    const product = (new Sanity).findByUidAndVersion(this.getUid(), this.version)
    if (!product) {
      return true
    } else if (!this.isVariable) {
      return false
    ...
    for (const [index, variation] of this.getVariations().instances.entries()) {
      const variant = (new Sanity).findByUidAndVersion(this.getUid(variation.name[this.lang]), this.version)
