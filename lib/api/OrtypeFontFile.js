...
export default class OrtypeFontFile extends OrtypeAbstractFont {
   ...
   * @returns {Promise<*[]>}
   */
  async getFontProductData(force = false) {
    const products = []
    ...
    const fontFileAsSlug = this.file.match(/.*\/([^\/]*)\.(ttf|otf)/)[1]
    ...
    if (this.isVariable) {
      ...
      for (const [
        ...
        if (variation?.name?.en) {
          ...
          const variant = {
            ...
            slug: {
              current: slugify(
                `${fontFileAsSlug} ${variation.name[this.lang]}`,
                { lower: true }
              ),
            },
      ...
      const variant = {
        ...
        slug: {
          current: slugify(
            `${fontFileAsSlug} ${this.getInfo().fullName[this.lang]}`,
            { lower: true }
          ),
        },
