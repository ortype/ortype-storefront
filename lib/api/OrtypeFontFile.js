...
export default class OrtypeFontFile extends OrtypeAbstractFont {
  ...
  async shouldUpdate() {
    ...
    //   }]
    // })
    const product = Sanity.getInstance().findByUidAndVersion(this.getUid(), this.version)
    if (!product) {
      return true
    } else if (!this.isVariable) {
      return false
    ...
    for (const [index, variation] of this.getVariations().instances.entries()) {
      const variant = Sanity.getInstance().findByUidAndVersion(this.getUid(variation.name[this.lang]), this.version)
  ...
  async getReactionProducts(force = false) {
    ...
    const parentProduct = await this.getFontProductData({
      ...
      handle: slugify(this.familyName)
    })
