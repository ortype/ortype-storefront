...
export const PaymentDetails: React.FC<Props> = ({ hasEditButton = false }) => {
  ...
  const { isCreditCard } = checkoutCtx
      ...
      {isCreditCard && hasEditButton && (
        <div className="ml-10 lg:ml-3">
          <PaymentSourceEditButton
            ...
            className="text-sm font-bold leading-none border-b border-black text-base border-opacity-10 md: transition ease-in duration-200 hover:border-opacity-50 hover:text-base focus:outline-none"
          />
        </div>
      )}
    </Fragment>
