...
export default async function config(
  ...
  try {
    ...
    const addresses = await cl.addresses.list({
      filters: { company_eq: 'Or Type' },
    })
    let address
    if (addresses.length) {
    ...
    const merchants = await cl.merchants.list({
      filters: { name_eq: 'Or Type' },
    })
    let merchant
    if (merchants.length) {
    ...
    const inventoryModels = await cl.inventory_models.list({
      filters: { name_eq: 'Main' },
    })
    let inventoryModel
    ...
    const inventoryStockLocations = await cl.inventory_stock_locations.list({
      filters: {
    ...
    const priceLists = await cl.price_lists.list({
      filters: { name_eq: 'Base Price' },
    })
    let priceList
