...
export default class OrType {
  ...
  constructor(file) {
    this.lang = 'en'
    this.file = file
    this.name = getPathInfo(file).name
  ...
  fileSlug() {
    return slugify(this.familyName, { lower: true }) // this is safer that filename because we do file merging
