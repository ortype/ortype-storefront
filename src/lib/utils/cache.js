 ...
 */
const cacheGet = (key, fallback = undefined) => {
  return cacheHas(key) ? cacheStore[key].value : fallback
}
 ...
 */
const cacheSet = (key, value, expiry = DEFAULT_EXPIRY) => {
  cacheStore[key] = {
    value,
...
export const cacheDetails = () => {
  ...
  const { totalSize, validSize } = Object.values(cacheStore).reduce(
    (acc, entry) => {
      const entrySize = sizeInMB(entry)
