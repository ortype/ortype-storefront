...
const updateEverything = async (data, inputObject) => {
  ...
  console.log('updatedEverything: data', data)
  ...
  */
  const productObject = {
    ...
    'shopify.tags': data.tags,
    'shopify.productType': data.product_type,
    'shopify.defaultPrice': data.variants[0].price,
    'shopify.defaultVariant.title': data.variants[0].title,
    'shopify.defaultVariant.price': data.variants[0].price,
    'shopify.defaultVariant.sku': data.variants[0].sku,
    'shopify.defaultVariant.variantId': data.variants[0].id,
    'shopify.defaultVariant.taxable': data.variants[0].taxable,
    'shopify.defaultVariant.inventoryQuantity': data.variants[0].inventory_quantity,
    'shopify.defaultVariant.inventoryPolicy': data.variants[0].inventory_policy,
    'shopify.defaultVariant.barcode': data.variants[0].barcode,
  ...
  const updateMetaField = await axios({
    url: `https://${SHOPIFY_API_KEY}:${SHOPIFY_ADMIN_PASSWORD}@${SHOPIFY_STORE_URL}/admin/api/2022-01/graphql.json`,
    method: 'POST',
...
export const handler = async (event) => {
  ...
  console.log('shopify-sync handler!', event.httpMethod, event.body)
  ...
  try {
    ...
    const generatedHash = crypto.createHmac('sha256', SHOPIFY_WEBHOOK_SECRET)
      .update(event.body)
      .digest('base64')
    console.log('generatedHash: ', generatedHash, hmac)
  ...
  if (data.hasOwnProperty('title') && data.hasOwnProperty('handle')) {
    ...
    try {
      ...
      console.log('url: ', `https://${SHOPIFY_API_KEY}:${SHOPIFY_ADMIN_PASSWORD}@${SHOPIFY_STORE_URL}/admin/api/2022-01/graphql.json`)
      ...
      // data.data.node is sometimes undefined

      console.log('trying with shopifyProduct: ', id, title, handle)
  ...
  // move all of this inside of the fetch request to block builds when not necessary
    ...
    console.log('did not pass conditions... data:', data)
  }
};
